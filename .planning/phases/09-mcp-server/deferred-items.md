...
## [contracts] delegate_parallel promises worktree isolation it does not have (found 2026-07-03)

`src/contracts/delegate_parallel.ts:23` defaults `isolation` to `"worktree"` and describes per-task git worktrees with merge-back — but no code implements it (the engine existed only in the relay-mcp predecessor; see the port map). The schema is a lying default. Fix at merge time, one of: implement (only if a real trigger fires — session/agent worktrees are already covered natively by Claude Code + `git worktree`, and this is neither dispatch, memory, nor receipt), or flip the default to `"none"` with an honest description. Second option recommended.
