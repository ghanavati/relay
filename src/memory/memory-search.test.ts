...
function parseResult(result: ReturnType<typeof handleMemorySearch>): {
...
describe('SHIP-54: memory_search progressive disclosure', () => {
  ...
  test('does NOT increment recall_count (excerpt is not content disclosure)', () => {
    const store = new MemoryStore();
    ...
    handleMemorySearch({ token_budget: 2000, tags: ['probe'], workdir, include_expired: false });
  ...
  test('excerpt of short content has no ellipsis', () => {
    const store = new MemoryStore();
    const workdir = '/ship54-short-' + Date.now();
    store.remember({ content: 'short', memory_type: 'fact', workdir, tags: ['shorttag'] });

    const parsed = parseResult(handleMemorySearch({ token_budget: 2000, tags: ['shorttag'], workdir, include_expired: false }));
    assert.strictEqual(parsed.results.length, 1);
    assert.strictEqual(parsed.results[0]!.excerpt, 'short');
  });

  test('surfaces trust_level in the compact index', () => {
    const store = new MemoryStore();
    ...
    const parsed = parseResult(handleMemorySearch({ token_budget: 2000, tags: ['trusttag'], workdir, include_expired: false }));
    assert.strictEqual(parsed.results.length, 1);
