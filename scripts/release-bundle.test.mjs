...
test('creates a self-contained macOS arm64 release plan', () => {
  ...
  assert.equal(plan.archiveName, 'relay-v0.4.0-darwin-arm64.tar.gz');
...
test('rejects unsupported release targets', () => {
  assert.throws(
    () => createBundlePlan({ version: '0.4.0', platform: 'win32', arch: 'x64' }),
    /Unsupported release target: win32-x64/,
  );
});

test('release workflow builds the supported archives and publishes a release', async () => {
